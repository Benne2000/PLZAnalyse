...
class GeoMapWidget extends HTMLElement {
  ...
  getFilteredData() {
    // Bei Split-Binding-Strategie: Detail-Daten sind in myDetailSource
    const src = (this._myDetailSource?.state === "success")
      ? this._myDetailSource
      : this._myDataSource;
    if (!src || src.state !== "success") return [];
    const {erhID,jahr,nummer}=this._activeFilter||{};
    // Wenn Detail-Source aktiv: alle Zeilen sind bereits gefiltert (SAC-Filter)
    if (src === this._myDetailSource) {
      const data = src.data || [];
    ...
    // Fallback: Index-Lookup aus Binding 1
  ...
  async loadErhebung(erhID, jahr, nummer) {
    ...
    // Detail-Binding zurücksetzen damit der Poll nicht einen alten "success"-State sieht
    // (SAC aktualisiert myDetailSource asynchron nach Filteränderung)
    this._detailSourceReady = false;
    // Laufenden Detail-Poll stoppen falls noch aktiv
    if (this._detailPollTimer) { clearInterval(this._detailPollTimer); this._detailPollTimer = null; }

    this._showCinematicLoader();
    try {
      // Phase 1: Detail-Daten laden (wartet auf myDetailSource ODER nutzt Index-Fallback)
      this._updateLoaderPhase(1, "Lade Erhebungsdaten…");

      // GeoJSON und Detail-Query parallel starten
  ...
  async queryErhebungFromBW(erhID, jahr, nummer) {
    ...
    // Warten auf Detail-Binding
    this._updateLoaderPhase(1, "Lade Erhebungsdaten…");
    return new Promise((resolve, reject) => {
      this._scheduleDetailPoll(resolve, reject);
    });
  }
  ...
  set myDetailSource(dataBinding) {
    this._myDetailSource = dataBinding;
    // Wenn ein Detail-Poll läuft und die Daten jetzt da sind → resolve
    // Der Poll selbst prüft state und ruft resolve auf – nichts zu tun hier.
    // Nur loggen für Debugging:
    // console.debug("[GeoMap] myDetailSource state:", dataBinding?.state, "rows:", dataBinding?.data?.length);
  }
  ...
  _scheduleDataPoll() {
    ...
    this._dataPollTimer = setInterval(() => {
      if (this._myDataSource?.state === "success") {
        ...
        // Flicker-Fix: Sekundenanzeige nur 1x pro Sekunde aktualisieren
  ...
  // Poll für myDetailSource (Kennzahl-Binding) nach Erhebungsauswahl
  _scheduleDetailPoll(resolve, reject) {
    ...
    this._detailPollTimer = setInterval(() => {
      if (this._myDetailSource?.state === "success") {
        ...
        if (secs !== _lastSecs) {
          _lastSecs = secs;
          this._updateLoaderPhase(1, `Lade Erhebungsdaten… (${secs}s)`);
        }
      }
    }, 50);
  }
